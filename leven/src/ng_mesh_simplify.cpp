...
public:
	...
	{
		base_ = static_cast<T*>(malloc(sizeof(T) * capacity));
		end_ = base_ + capacity;
	}
	...
	LinearBuffer<Edge>& edges)
{
	for (int i = 0; i < triangles.size(); i++)
	{
		...
		edges.push_back(Edge(glm::min(indices[0], indices[1]), glm::max(indices[0], indices[1])));
		edges.push_back(Edge(glm::min(indices[1], indices[2]), glm::max(indices[1], indices[2])));
		edges.push_back(Edge(glm::min(indices[0], indices[2]), glm::max(indices[0], indices[2])));
	}
	...
	LinearBuffer<vec4>& collapseNormal)
{
	...
	for (int i: randomEdges)
	{
		...
		const float cosAngle = glm::dot(vMin.normal, vMax.normal);
		if (cosAngle < options.minAngleCosine)
		{
			continue;
		}

		const float edgeSize = glm::length2(vMax.xyz - vMin.xyz);
		if (edgeSize > (options.maxEdgeSize * options.maxEdgeSize))
		{
			continue;
		}

		if (glm::abs(vMin.colour[3] - vMax.colour[3]) > 1e-3)
		{
			continue;
		}
		...
		const int penalty = glm::max(0, degree - 10);
		...
		collapsePosition[i] = vec4(pos[0], pos[1], pos[2], 1.f);
	...
	LinearBuffer<int>& collapseTarget)
{
	...
	for (int i: collapseValid)
	{
		...
		if (collapseEdgeID[edge.min_] == i && collapseEdgeID[edge.max_] == i)
		{
			...
			vertices[edge.min_].xyz = collapsePositions[i];
	...
	const MeshSimplificationOptions& options)
{
	...
	for (int i = 0; i < vertices.size(); i++)
	{
		mesh->vertices[i].xyz = vertices[i].xyz + worldSpaceOffset;
		mesh->vertices[i].normal = vertices[i].normal;
		mesh->vertices[i].colour = vertices[i].colour;
	}
}
